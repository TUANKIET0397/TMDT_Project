...
class AdminController {
    ...
    async dashboardData(req, res) {
        try {
          ...
          WHERE tp.TypeName = 'Clothes'
            ...
            const [totalRevenueThisYear, totalRevenueGrowthYoY] =
                await Promise.all([
            ...
            return res.json({
                ...
                productsByType,
                invoices,
                recentInvoices: Array.isArray(invoices)
                    ? invoices.slice(0, 7)
                    : [],
            })
        } catch (err) {
        ...
        try {
            ...
            const saveUploadedFiles = async (filesArray, prefix = "file") => {
                ...
                for (const f of filesArray) {
                    const name = `${prefix}_${Date.now()}_${
                        f.originalname
                    }`.replace(/\s+/g, "_")
            ...
            if (req.files) {
                if (Array.isArray(req.files)) {
                    req.files.forEach((f) => {
                        const m =
                            f.fieldname &&
                            f.fieldname.match(/^colors\[(\d+)\]\[images\]$/)
                        if (m) {
                            const idx = m[1]
                            if (!colorImagesGroups[idx])
                                colorImagesGroups[idx] = []
            ...
            for (const [index, colorData] of Object.entries(colorsData)) {
                ...
                const sizes = Array.isArray(rawSizes)
                    ? rawSizes
                          .filter(
                              (s) => s && (s.size || s.quantity !== undefined)
                          )
                          .map((s) => ({
    ...
    async exportCSV(req, res) {
        try {
            ...
            ).toLocaleString()}",${(stats.monthlyUsersGrowth || 0).toFixed(
                1
            )}%\n`
            csv += `New Sign Ups,"${(
                stats.newSignUps || 0
            ).toLocaleString()}",${(stats.signUpsGrowth || 0).toFixed(1)}%\n`
            csv += `Total Invoices,"${(
                stats.totalInvoices || 0
            ).toLocaleString()}",${(stats.totalInvoicesGrowth || 0).toFixed(
                1
            )}%\n`
            csv += `Products Sold,"${(
                stats.totalProductsSold || 0
            ).toLocaleString()}",${(stats.totalProductsGrowth || 0).toFixed(
                1
            )}%\n`
            ...
            monthlyRevenue.forEach((m) => {
                const monthName = new Date(0, m.month - 1).toLocaleString(
                    "en",
                    {
                        month: "long",
                    }
            ...
            invoices.slice(0, 50).forEach((inv) => {
                ...
                const name = `${inv.FirstName} ${inv.LastName}`.replace(
                    /,/g,
                    ""
                )
    ...
    async exportExcel(req, res) {
        try {
            ...
            monthlyRevenue.forEach((m) => {
                ...
                const revenue = `${Number(m.amount || 0).toLocaleString(
                    "en-US",
                    {
                        minimumFractionDigits: 2,
                    }

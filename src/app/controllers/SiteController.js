...
class SiteController {
  ...
  async profile(req, res) {
    try {
      ...
      const regionsWithSelection = regions.map((region) => ({
        ...region,
        selected:
          String(region.ID) === String(user.RegionID || user.region || ''),
      }));
  ...
  async checkout(req, res) {
    try {
      ...
      const userId = req.session.userId;
      const user =
        req.user || (userId ? await AuthSite.getUserById(userId) : null) || {};
      ...
      if (req.query.cartData) {
        try {
          ...
          console.warn('⚠️ Failed to parse cartData from query:', err.message);
          cartItems = [];
        }
      }
    ...
    async payment(req, res) {
      ...
      } else if (req.session.cartItems && req.session.cartItems.length > 0) {
      ...
      if (paymentType === 'momo' && amount) {
        try {
          ...
          if (paymentResult.payUrl) {
            ...
            return res.status(400).render('error', {
              layout: 'payment',
              message: 'Không thể kết nối với cổng thanh toán MoMo.',
      ...
      if (!paymentType || paymentType === 'cod') {
        try {
          ...
          try {
            const cartID = await Invoice.createCartFromItems(userId, cartItems);

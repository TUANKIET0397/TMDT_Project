...
class AuthSite {
  ...
  static async loginAdmin(adminName, password) {
    try {
      ...
      if (isBcryptHash) {
        // Nếu là bcrypt hash
        console.log('🔐 Verifying admin password with bcrypt for:', adminName);
        ...
        if (isValidPassword) {
          ...
          try {
            const saltRounds = 10;
            const hashedPassword = await bcrypt.hash(password, saltRounds);
            await db.query('UPDATE Admins SET PasswordHash = ? WHERE ID = ?', [
              hashedPassword,
              admin.ID,
            ]);
            console.log(
  ...
  static async changeAdminPassword(adminId, oldPassword, newPassword) {
    ...
    try {
      ...
      if (isBcryptHash) {
        isValidPassword = await bcrypt.compare(oldPassword, admin.PasswordHash);
      } else {

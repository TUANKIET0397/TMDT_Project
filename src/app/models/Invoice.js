...
class Invoice {
          ...
          try {
            await connection.query(
              `INSERT INTO CartItem (CartID, SizeID, ColorID, ProductID, Volume, UnitPrice, TotalPrice)
                             VALUES (?, ?, ?, ?, ?, ?, ?)`,
              [cartID, sizeID, colorID, productId, quantity, price, totalPrice]
            );
  ...
  static async decreaseProductStock(cartItems) {
    ...
    try {
      ...
      console.log('📦 Starting stock decrease for', cartItems.length, 'items');
      ...
      for (const item of cartItems) {
        ...
        if (updateResult.affectedRows > 0) {
          ...
          console.warn(`⚠️ Failed to decrease stock for product ${productId}`);
        }
      }

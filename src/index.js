...
io.on('connection', (socket) => {
    ...
    try {
      ...
      if (!Number.isNaN(numericId)) {
        ...
        if (rows.length > 0) {
          ...
          displayName = fullName || rows[0].Email || `User #${numericId}`;
        }
      }

      socket.userName = displayName || `Guest ${String(rawUserId).slice(-4)}`;
  ...
  socket.on('user:message', async (data) => {
    ...
    if (!dbUserId || Number.isNaN(dbUserId)) {
      console.warn('⚠️ Blocked chat from unauthenticated user:', socket.id);
      return;
    }

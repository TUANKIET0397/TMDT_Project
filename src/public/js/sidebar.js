...
// Use a namespaced localStorage key for admin vs public pages to avoid
// cross-page state pollution (e.g., site product selection appearing in admin).
const SIDEBAR_KEY = (() => {
    try {
        const p = window.location && window.location.pathname
        // If path starts with /admin, use admin-scoped key
        if (p && /^\/admin/.test(p)) return "adminSidebarActive"
    } catch (e) {
        // ignore
    }
...
dashboardItems.forEach((item) => {
    item.addEventListener("click", function (e) {
        ...
        // Persist to localStorage (use namespaced key)
        const text = this.textContent.trim()
        if (text) localStorage.setItem(SIDEBAR_KEY, `dashboard:${text}`)
...
if (sidebarLogo) {
    sidebarLogo.addEventListener("click", function (e) {
        // clear active state from localStorage (namespaced)
...
window.addEventListener("load", () => {
    const savedActive = localStorage.getItem(SIDEBAR_KEY)
            ...
            datasets: [
                {
                    label: "Revenue",
                    data: [
                        0, 5000, 12000, 12500, 15000, 20000, 25000, 27000,
                        31000, 35000, 38000, 40000,
                    ],
                    borderColor: "#CB3CFF",
            ...
            datasets: [
                {
                    label: "Outerwear",
                    data: [
                        80, 120, 160, 150, 180, 220, 300, 270, 310, 250, 200,
                        220,
                    ],
                    borderColor: "#CB3CFF",

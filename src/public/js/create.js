...
function clearActiveSizes() {
  document
    .querySelectorAll('.section__form-size.active-form')
    .forEach((el) => el.classList.remove('active-form'));
  document
    .querySelectorAll('.modal-size-btn.active')
...
function addColorCard(index) {
  ...
  if (firstImg) {
    ...
    imgEl.src =
      firstImg instanceof File ? URL.createObjectURL(firstImg) : firstImg;
...
function renderColorCards() {
  ...
  const addTemplate = colorCardsContainer.querySelector(
    '.add__product-color:not([data-color-index])'
  );
...
function updateQuantityVisibility() {
  ...
  const selected = Array.from(
    document.querySelectorAll('.section__form-size')
  ).find(
    (el) =>
      el.classList.contains('active-form') &&
...
if (colorCardsContainer) {
  colorCardsContainer.addEventListener('click', (e) => {
    ...
    if (typeof idx === 'undefined') {
      // ← KHI THÊM COLOR MỚI: XÓA COLOR "Default" NẾU CÓ
      productData.colors = productData.colors.filter(
        (c) => c.colorName !== 'Default'
      );
      ...
      tempColorData = JSON.parse(
        JSON.stringify(
          productData.colors[editingColorIndex] || {
            colorName: '',
            images: [],
            sizes: [],
          }
...
if (sizesContainer) {
  sizesContainer.addEventListener('click', function (e) {
    ...
    document
      .querySelectorAll('.section__form-size')
      .forEach((opt) => opt.classList.remove('active-form'));
...
closeModalBtn.addEventListener('click', function () {
  ...
  if (
    ...
    if (card) {
      ...
      if (plusBlock) {
        const first = (tempColorData.images && tempColorData.images[0]) || null;
        let src = '';
        if (first) {
...
saveBtn.addEventListener('click', async function () {
  ...
  try {
    ...
    if (res.ok && data.success) {
      alert(
        'Product saved. ID: ' + (data.productID || data.productId || 'unknown')
      );
